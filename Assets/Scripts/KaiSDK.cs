...
namespace Kai.SDK
{
	public static class KaiSDK
	{
		...
		/// <summary>
		/// Sets the hand of the given Kai
		/// </summary>
		public static void SetHand(Kai kai, Hand hand)
		{
			kai.Hand = hand;
			...
			var json = new JObject
			{
				[Constants.Type] = Constants.SwitchHand
			};

			if (ReferenceEquals(kai, DefaultKai))
			{
				json.Add(Constants.KaiID, Constants.DefaultKai);
			}
			else if (ReferenceEquals(kai, DefaultLeftKai))
			{
				json.Add(Constants.KaiID, Constants.DefaultLeftKai);
			}
			else if (ReferenceEquals(kai, DefaultRightKai))
			{
				json.Add(Constants.KaiID, Constants.DefaultRightKai);
			}
			else
			{
				json.Add(Constants.KaiID, kai.KaiID);
			...
			json.Add(Constants.Hand, kai.Hand.ToString().ToLower());
		...
		private static void DecodeKaiConnected(JObject input)
		{
			...
			if (defaultKai == true)
			{
				...
				if (DefaultKai.Hand == 0)
				{
					DefaultKai.Hand = handEnum;
				}
				else
				{
					SetHand(DefaultKai, DefaultKai.Hand);
					handEnum = DefaultKai.Hand;
				}
			}
